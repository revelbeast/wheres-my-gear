...
root = Path.cwd()
security_dir = root / "components" / "security"
security_dir.mkdir(parents=True, exist_ok=True)

gate_path = security_dir / "AppLockGate.tsx"
gate_path.write_text('''import AsyncStorage from "@react-native-async-storage/async-storage";
...
if 'import AppLockGate from "../components/security/AppLockGate";' not in text:
    text = text.replace(
...
if old not in text:
    raise SystemExit("Could not find SyncProvider RootLayoutInner anchor")

text = text.replace(old, new)
layout_path.write_text(text)

print("Added AppLockGate and wrapped RootLayoutInner.")
